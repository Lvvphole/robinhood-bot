# Generated Repository Map

Status: GENERATED
Owner: tools/generate_repo_map.py
Last verified: generated at commit/worktree state

Do not hand-edit. Regenerate with `python tools/generate_repo_map.py`.

- `.gitattributes`
- `.github/workflows/repo-contract.yml`
- `.github/workflows/repo-garden.yml`
- `AGENTS.md`
- `ARCHITECTURE.md`
- `README.md`
- `STATUS.md`
- `config/baseline.yaml`
- `docs/DESIGN.md`
- `docs/PLANS.md`
- `docs/PRODUCT_SENSE.md`
- `docs/QUALITY_SCORE.md`
- `docs/RELIABILITY.md`
- `docs/SECURITY.md`
- `docs/design-docs/core-beliefs.md`
- `docs/design-docs/index.md`
- `docs/design-docs/multifactor-graph.md`
- `docs/exec-plans/active/agent-first-graph-migration.md`
- `docs/exec-plans/completed/index.md`
- `docs/exec-plans/tech-debt-tracker.md`
- `docs/product-specs/index.md`
- `docs/product-specs/multifactor-research-system.md`
- `docs/references/evidence-index.md`
- `docs/references/harness-engineering.md`
- `examples/data_schema.md`
- `pyproject.toml`
- `src/zero_dte_bot/__init__.py`
- `src/zero_dte_bot/backtest.py`
- `src/zero_dte_bot/broker.py`
- `src/zero_dte_bot/cli.py`
- `src/zero_dte_bot/config.py`
- `src/zero_dte_bot/execution.py`
- `src/zero_dte_bot/greeks.py`
- `src/zero_dte_bot/indicators.py`
- `src/zero_dte_bot/metrics.py`
- `src/zero_dte_bot/models.py`
- `src/zero_dte_bot/monte_carlo.py`
- `src/zero_dte_bot/robinhood_gateway.py`
- `src/zero_dte_bot/strategy.py`
- `src/zero_dte_bot/validation.py`
- `tests/test_backtest_smoke.py`
- `tests/test_config.py`
- `tests/test_greeks.py`
- `tests/test_repo_contract.py`
- `tests/test_validation.py`
- `tools/generate_repo_map.py`
- `tools/lint_repo_knowledge.py`
